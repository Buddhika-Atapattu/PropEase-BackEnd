...
export default class TrafficMonitor {
        ...
        app.use( ( req: Request, res: Response, next: NextFunction ) => {
            ...
            const onFinish = () => {
                ...
                if ( this.echo ) {
                    ...
                    console.log(
                        '[LOG:]', `[${ this.tag }] [${ reqId }] HTTP ${ status } \n[${ req.method }] ${ req.originalUrl } ${ durationMs }ms ${ ip }${ clen }`, '\n'
                    );
                }
            };
                    ...
                    try {
                        if ( typeof firstArg === 'string' ) {
                            ...
                            if ( self.echo ) {
                                ...
                                console.log(
                                    '[LOG:]', `[${ self.tag }] \n[Route:] ${ label.toUpperCase() } ${ m.toUpperCase() } ${ firstArg }${ where ? ` @ ${ where }` : '' }`, '\n'
                                );
                            }
                        }
                    } catch {
    ...
    installSocket( io: Namespace ) {
        io.on( 'connection', ( socket: Socket ) => {
            ...
            if ( this.echo ) {
                ...
                console.log( '[LOG:]', `[${ this.tag }] [${ reqId }] SOCKET CONNECT id=${ socket.id } ip=${ ip }`, '\n' );
            }
            ...
            ( socket as any ).onevent = ( packet: any ) => {
                try {
                    ...
                    if ( this.echo && eventName ) {
                        ...
                        console.log( '[LOG:]', `[${ this.tag }] [${ reqId }] SOCKET IN  id=${ socket.id } event=${ eventName } size=${ size ?? 0 }B`, '\n' );
                    }
                } catch {
            ...
            socket.emit = ( event: string, ...args: any[] ) => {
                try {
                    ...
                    if ( this.echo && event ) {
                        ...
                        console.log( '[LOG:]', `[${ this.tag }] [${ reqId }] SOCKET OUT id=${ socket.id } event=${ event } size=${ size ?? 0 }B`, '\n' );
                    }
                } catch {
    ...
    private writeLine( file: string, obj: unknown ) {
        ...
        try {
            ...
            console.error( '[Error:]🚫 [TrafficMonitor] write failed:', e, '\n' );
        }
    }
