...
type AuthedReq = Request & {user: {username: string; role: Role}};
...
export default class NotificationController {
    ...
    private listMine: RequestHandler = async (req, res) => {
        try {
            const {username, role} = (req as AuthedReq).user;
    ...
    private create: RequestHandler = async (req, res) => {
        try {
            const allowedRoles: Role[] = ['admin', 'operator', 'manager'];
            const {role} = (req as AuthedReq).user;
            if(!allowedRoles.includes(role)) {
                res.status(403).json({message: 'Permission denied'});
                return;
                // note: return to stop execution
            }
            ...
            const created = await this.service.createNotification(
                ...
                (rooms, payload) => {
                    rooms.forEach((room) => {
                        this.sockets.instance.to(room).emit('notification.new', payload);
                    });
                }
            );
    ...
    private markRead: RequestHandler = async (req, res) => {
        try {
            const {username} = (req as AuthedReq).user;
            const {id} = req.params;
