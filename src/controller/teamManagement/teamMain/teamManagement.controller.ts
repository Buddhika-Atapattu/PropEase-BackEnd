...
import { FileMetaPacket, type PaginationMeta } from "../../../types/api-message";
...
export class TeamManagementController {
  ...
  public async createTeam(req: Request, res: Response): Promise<void> {
    try {
      ...
      ApiResponseBuilder.ok(res, "team", responseTeam as unknown as never, "Team created successfully");
      // DO NOT return res.json(...) directly (your rule)
      // res already written by ApiResponseBuilder
      // but keep the Promise<void> style:
      // return;
      ...
      try {
        ...
        await notificationService.createNotification(
          {
            title: "New Team",
            body: `A new team "${teamName}" has been created.`,
            type: "create",
            severity: "info",
            audience: { mode: "role", roles: ["admin", "manager", "operator"] },
            channels: ["inapp", "email"],
            metadata: { refId: teamCode, data: { team: responseTeam } },
          },
          (rooms, payload2) => rooms.forEach((r) => io.to(r).emit("notification.new", payload2))
        );
      } catch (notifyErr) {
        console.error("[Warning:] [TeamManagementController:createTeam] Notification failed.\n", notifyErr);
      }
  ...
  public async listTeams(req: Request, res: Response): Promise<void> {
    try {
      ...
      ApiResponseBuilder.ok(res, "teams", result.rows as unknown as never, "Teams fetched successfully", {
  ...
  public async updateTeam(req: Request<{ teamCode: string }>, res: Response): Promise<void> {
    try {
      ...
      const enriched = await this.rest.fetchEnrichedTeamByCode(teamCode);
      const responseTeam = enriched ?? (updated as unknown);

      ApiResponseBuilder.ok(res, "team", responseTeam as never, "Team updated successfully");
      ...
      try {
        ...
        await notificationService.createNotification(
          {
            title: "Update Team",
            body: `Team "${String((updated as unknown as { teamName?: unknown })?.teamName ?? "")}" has been updated.`,
            type: "update",
            severity: "info",
            audience: { mode: "role", roles: ["admin", "manager", "operator"] },
            channels: ["inapp", "email"],
        ...
        console.error("[Warning:] [TeamManagementController:updateTeam] Notification failed.\n", notifyErr);
      }
  ...
  public async deleteTeam(req: Request<{ teamCode: string }>, res: Response): Promise<void> {
    try {
      ...
      // We need teamId for WS payload when hard delete; try to read the doc first.
      const existing = await TeamManagementModel.findOne({ teamCode }).lean().exec();
      if (!existing) {
      ...
      if (soft) {
        ...
        // WS: soft delete still counts as update for list invalidation
        const ctx = await this.buildWsContext(req);
      ...
      ApiResponseBuilder.ok(res, "team", deleted as unknown as never, "Team deleted permanently");

      const ctx = await this.buildWsContext(req);
      await this.ws.emitDeleted(ctx, teamCode, String((existing as unknown as { _id?: unknown })?._id ?? ""));
