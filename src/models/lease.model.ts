...
import { AddedBy, Property } from './property.model'; // used in LeasePayloadWithProperty type
...
export class LeaseModelBuilder {
  public static buildSchema(): Schema<LeaseType> {
    ...
    const CurrencyFormatSchema = new Schema<CurrencyFormat>(
      {
        ...
        symbol: { type: String, required: true }, // align with interface
        flags: {
    ...
    const RentDueDateSchema = new Schema<RentDueDate>(
      {
        ...
        offsetDays: { type: Number, required: false, default: 0 },
    ...
    // ───────────────── AddedBy (from property.model.ts) ─────────────────
    // Shape:
    //   username: string;
    //   name: string;
    //   email: string;
    //   role: 'admin' | 'agent' | 'owner' | string;
    //   contactNumber?: { code: CountryCodes; number: string };
    //   addedAt: Date | string;
    const AddedByContactNumberSchema = new Schema(
      {
        code: {
    ...
    const AddedBySchema = new Schema<AddedBy>(
      {
        ...
        contactNumber: {
          type: AddedByContactNumberSchema,
          required: false,
          default: undefined,
        },
        addedAt: {
    ...
    const LeaseAgreementSchema = new Schema<LeaseAgreement>(
      {
        ...
        latePaymentPenalties: {
          type: [ LatePaymentPenaltySchema ],
          required: true,
          default: [],
        },
        utilityResponsibilities: {
          type: [ UtilityResponsibilitySchema ],
          required: true,
          default: [],
        },
        noticePeriodDays: {
          type: NoticePeriodSchema,
          required: true,
          default: {},
        },
      },
    ...
    // ───────────────── Rules, signatures, system metadata ─────────────────
