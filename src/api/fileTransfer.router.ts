    ...
    this.router.post(
      ...
      async ( req: Request<{ token: string; }>, res: Response ) => {
        try {
          ...
          if ( !tenant || !issuedAt ) {
            ApiResponseBuilder.validationError( res, 'Invalid token payload.' );
            return;
          }
          ...
          if ( !file ) {
            ApiResponseBuilder.validationError( res, 'Image is required.' );
            return;
          }
          ...
          ApiResponseBuilder.ok( res, "other", { record }, 'File uploaded successfully' );
          return;
        } catch ( error ) {
    ...
    this.router.get(
      ...
      async ( req: Request<{ tenant: string; }>, res: Response ) => {
        try {
          ...
          if ( !tenant ) {
            ApiResponseBuilder.validationError( res, 'Tenant is required!' );
            return;
          }
          ...
          ApiResponseBuilder.ok( res, 'other', other, 'File fetched successfully!' );
          return;
        } catch ( error ) {
    ...
    this.router.post( "/convert-to-pdf", async ( req: Request, res: Response ) => {
      try {
        ...
        if ( !fileUrl ) {
          ...
          ApiResponseBuilder.validationError( res, 'File URL is required!' );
          return;
        }
        ...
        libre.convert( fileBuffer, ".pdf", undefined, ( err, done ) => {
          if ( err ) {
            ...
            ApiResponseBuilder.internalError( res, 'PDF conversion failed!' );
            return;
          }
        ...
        ApiResponseBuilder.internalError( res, error instanceof Error ? error.message : 'PDF conversion failed!' );
        return;
      }
    } );
  }
